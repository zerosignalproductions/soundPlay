...
listen2EdmServices.factory('API', [
  '$resource',
  'appConfig',
  function ($resource, appConfig) {
    return {
      ...
      Explore_Tracks: $resource(appConfig.soundcloud.explore_tracks_url + '/:category', {
        limit: '10',
        category: '@category',
        consumer_key: appConfig.soundcloud.consumer_key
      }, {
        get: {
          method: 'GET',
          params: {}
      ...
      Tag: $resource('//api.soundcloud.com/tracks', { consumer_key: appConfig.soundcloud.consumer_key }, {
        get: {
          method: 'GET',
          params: { consumer_key: appConfig.soundcloud.consumer_key },
          isArray: true
        }
      }),
      TrackStream: $resource('//api.soundcloud.com/resolve.json', { client_id: appConfig.soundcloud.consumer_key }, {
        get: {
          method: 'GET',
          params: { client_id: appConfig.soundcloud.consumer_key }
        }
      }),
      Tracks: $resource('assets/testTracks.json', {}, {
        get: {
          method: 'GET',
          params: { key: appConfig.soundcloud.consumer_key }
        }
      })
    };
  }
]);
listen2EdmServices.factory('playlistFactory', [
  'appConfig',
  function (appConfig) {
    return {
      updatePlaylistArtwork: function (data, isSoundCloud) {
        _.each(data, function (value, key) {
          if (value.artwork_url !== null && isSoundCloud && value.streamable) {
            value.artwork_url_large = value.artwork_url.split('-').slice(0, -1).join('-') + value.artwork_url.split('-').slice(-1)[0].replace('large', '-t500x500');
          } else if (typeof value.soundcloud !== 'undefined' && value.soundcloud.streamable) {
            value.soundcloud.artwork_url_large = value.soundcloud.artwork_url.split('-').slice(0, -1).join('-') + value.soundcloud.artwork_url.split('-').slice(-1)[0].replace('large', '-t500x500');
          }
        });
      },
